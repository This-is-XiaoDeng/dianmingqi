...
- 📂 **多数据源**：
  - **txt**：一行一个名字；
  - **xlsx**：选择一整列，可通过复选框决定是否删除表头，支持指定列号与工作表。
- 🔖 **导入预览**：导入成功后弹出提示窗（非 alert），显示导入人数与前三个名字预览（不足三个显示全部）。
...
1. 点击 **📂 导入名单** 选择 `txt` 或 `xlsx` 文件。
   - `txt`：一行一个名字，空行自动忽略。
   - `xlsx`：默认读取第 1 列并**去除表头**（取消「删除表头」复选框可保留第一行）；可在导入面板输入列号选择其它列。
   - 导入成功后弹出提示窗，展示导入人数与前三个名字预览。
